...
/**
 * TinaCMS configuration for the CLEM LTD website.
 *
 * Editors manage everything below from a visual editor at  /admin
 * (e.g. https://clemltd.store/admin) after logging in via Tina Cloud.
 * Tina Cloud holds the GitHub connection, so editors are invited by
 * EMAIL and DO NOT need their own GitHub accounts.
 *
 * Saving in the editor commits content/site.json (and any uploaded
 * photos) to GitHub, which triggers an automatic Vercel redeploy.
 *
 * The website itself reads content/site.json at runtime, so the schema
 * below just mirrors that file.
 */
          ...
          },
          {
            ...
            fields: [
              ...
              { type: "string", name: "secondaryCta", label: "Secondary button text" },
              {
                ...
                label: "Key numbers",
                list: true,
                ui: { itemProps: (i: any) => ({ label: i?.number ? i.number + " — " + (i.label || "") : "Badge" }) },
                fields: [
          ...
          },
          {
            ...
            fields: [
              ...
              { type: "string", name: "paragraphs", label: "Paragraphs (you may use <strong> for bold)", list: true, ui: { component: "textarea" } },
              {
                ...
                label: "Stat cards",
                list: true,
                ui: { itemProps: (i: any) => ({ label: i?.number ? i.number + " — " + (i.label || "") : "Stat" }) },
                fields: [
                  { type: "string", name: "number", label: "Number" },
                  { type: "string", name: "label", label: "Label" },
          ...
          },
          {
            ...
            fields: [
              ...
              { type: "string", name: "intro", label: "Intro text", ui: { component: "textarea" } },
              {
                ...
                label: "Product list",
                list: true,
                ui: { itemProps: (i: any) => ({ label: i?.name || "Product" }) },
                fields: [
          ...
          },
          {
            ...
            fields: [
              ...
              { type: "string", name: "intro", label: "Intro text", ui: { component: "textarea" } },
              {
                ...
                list: true,
                ui: { itemProps: (i: any) => ({ label: i?.name || "Country" }) },
                fields: [
